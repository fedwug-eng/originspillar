        ...
        <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
                <div className="bg-card border border-border rounded-xl p-3">
                    ...
                    <div className="space-y-1">
                        {quickActions.map((a, i) => (
                            <button key={i} onClick={() => onNav(a.to)} className="w-full flex items-center justify-between p-2 rounded-lg hover:bg-accent transition-colors text-left">
                                <div className="flex items-center gap-2">
                                    <div className="w-6 h-6 rounded-lg bg-primary/10 flex items-center justify-center"><a.icon className="w-3 h-3 text-primary" /></div>
                                    <div><p className="text-[9px] font-medium text-foreground/80">{a.label}</p><p className="text-[7px] text-muted-foreground">{a.sub}</p></div>
                                </div>
                ...
                <div className="bg-card border border-border rounded-xl p-3">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-[8px] text-primary/60 cursor-pointer hover:text-primary">View all</span>
                    </div>
            ...
            <div className="bg-card border border-border rounded-xl p-3">
                <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1 bg-emerald-400/10 px-1.5 py-0.5 rounded"><TrendingUp className="w-3 h-3 text-emerald-400" /><span className="text-[8px] font-semibold text-emerald-400">+22%</span></div>
                </div>
                    ...
                    {revenueData.map((h, i) => {
                        ...
                        return (
                            <div key={i} className="flex-1 flex flex-col items-center gap-0.5 group">
                                <div className={`w-full rounded-sm transition-all group-hover:opacity-80 ${i >= 10 ? "bg-gradient-to-t from-primary to-primary/60" : i >= 8 ? "bg-primary/35" : "bg-muted"}`} style={{ height: `${pct}%` }} />
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                ...
                <button className="flex items-center gap-1 bg-primary text-white text-[9px] font-medium px-2.5 py-1.5 rounded-lg hover:bg-primary/90 transition-colors"><Plus className="w-3 h-3" /> New Project</button>
            </div>
            <div className="grid grid-cols-3 gap-2">
                {requests.map((r, i) => (
                    <div key={i} className="bg-card border border-border rounded-xl p-3 hover:border-primary/20 transition-all cursor-pointer group">
                        <div className="flex items-start gap-2 mb-2">
                            <div className="w-6 h-6 rounded-lg bg-primary/10 flex items-center justify-center shrink-0 mt-0.5"><FolderKanban className="w-3 h-3 text-primary" /></div>
                            <div className="min-w-0"><p className="text-[10px] font-semibold text-foreground truncate">{r.title}</p><p className="text-[7px] text-muted-foreground">{r.client}</p></div>
                        </div>
                        ...
                        <div className="mt-2">
                            ...
                            <div className="h-1 bg-muted rounded-full overflow-hidden"><div className="h-full bg-primary rounded-full transition-all" style={{ width: `${r.progress}%` }} /></div>
                        </div>
                        <div className="flex items-center justify-between mt-2">
                            <span className="text-[7px] text-muted-foreground flex items-center gap-0.5"><Clock className="w-2.5 h-2.5" /> Due {r.due}</span>
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                ...
                <button className="flex items-center gap-1 bg-primary text-white text-[9px] font-medium px-2.5 py-1.5 rounded-lg hover:bg-primary/90 transition-colors"><Plus className="w-3 h-3" /> Add Client</button>
            </div>
            <div className="bg-card border border-border rounded-xl overflow-hidden">
                ...
                {clients.map((c, i) => (
                    <div key={i} className="grid grid-cols-12 gap-2 px-3 py-2 items-center hover:bg-accent/50 transition-colors cursor-pointer border-b border-border/50 last:border-0">
                        <div className="col-span-3 flex items-center gap-2">
                            <div className={`w-6 h-6 rounded-lg ${c.color} flex items-center justify-center shrink-0`}><span className="text-[7px] font-bold text-white">{c.initials}</span></div>
                        ...
                        <div className="col-span-2 text-right">
                            <span className={`text-[7px] font-medium px-1.5 py-0.5 rounded ${c.status === "Active" ? "text-emerald-400 bg-emerald-400/10" : c.status === "New" ? "text-primary bg-primary/10" : "text-muted-foreground bg-muted"}`}>{c.status}</span>
                        </div>
                    </div>
                ))}
            </div>
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                ...
                <button className="flex items-center gap-1 bg-primary text-white text-[9px] font-medium px-2.5 py-1.5 rounded-lg hover:bg-primary/90 transition-colors"><Plus className="w-3 h-3" /> New Service</button>
            </div>
            <div className="grid grid-cols-3 gap-2">
            ...
            <div className="grid grid-cols-3 gap-2">
                {services.map((s, i) => (
                    <div key={i} className="bg-card border border-border rounded-xl p-3 hover:border-primary/20 transition-all cursor-pointer flex flex-col">
                        <div className="flex items-start gap-2 mb-1.5">
                            ...
                            <div className="min-w-0 flex-1">
                                <p className="text-[9px] font-semibold text-foreground truncate">{s.name}</p>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-border mt-auto">
                            <span className="text-[7px] text-muted-foreground"><Users className="w-2.5 h-2.5 inline mr-0.5" />{s.active} active</span>
                            <span className="text-[7px] text-emerald-400 cursor-pointer hover:text-emerald-300">✏️ Edit</span>
                        </div>
                    </div>
                ))}
            </div>
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                ...
                <button className="flex items-center gap-1 bg-primary text-white text-[9px] font-medium px-2.5 py-1.5 rounded-lg hover:bg-primary/90 transition-colors"><Plus className="w-3 h-3" /> Create Invoice</button>
            </div>
            <div className="grid grid-cols-3 gap-2">
                <div className="bg-card border border-border rounded-xl p-3"><p className="text-[8px] text-muted-foreground">Total Outstanding</p><p className="text-lg font-bold text-amber-400">$11,050</p><p className="text-[7px] text-muted-foreground/60">3 invoices</p></div>
                <div className="bg-card border border-border rounded-xl p-3"><p className="text-[8px] text-muted-foreground">Paid This Month</p><p className="text-lg font-bold text-emerald-400">$22,750</p><p className="text-[7px] text-muted-foreground/60">4 invoices</p></div>
                <div className="bg-card border border-border rounded-xl p-3"><p className="text-[8px] text-muted-foreground">Overdue</p><p className="text-lg font-bold text-rose-400">$3,200</p><p className="text-[7px] text-muted-foreground/60">1 invoice</p></div>
            </div>
            <div className="bg-card border border-border rounded-xl overflow-hidden">
                <div className="grid grid-cols-12 gap-2 px-3 py-2 text-[8px] font-semibold text-muted-foreground border-b border-border">
                    <div className="col-span-2">Invoice</div><div className="col-span-2">Client</div><div className="col-span-2">Project</div><div className="col-span-2 text-right">Amount</div><div className="col-span-2 text-center">Status</div><div className="col-span-2 text-right">Due</div>
                </div>
                {invoices.map((inv, i) => (
                    <div key={i} className="grid grid-cols-12 gap-2 px-3 py-2 items-center hover:bg-accent/50 transition-colors cursor-pointer border-b border-border/50 last:border-0">
                        <div className="col-span-2 text-[9px] font-medium text-foreground">{inv.id}</div>
                        <div className="col-span-2 text-[8px] text-muted-foreground truncate">{inv.client}</div>
                        <div className="col-span-2 text-[8px] text-muted-foreground truncate">{inv.project}</div>
                        <div className="col-span-2 text-[9px] font-medium text-foreground/80 text-right">{inv.amount}</div>
                        <div className="col-span-2 text-center"><span className={`text-[7px] font-medium px-1.5 py-0.5 rounded ${inv.statusColor}`}>{inv.status}</span></div>
                        <div className="col-span-2 text-[8px] text-muted-foreground text-right">{inv.due}</div>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="space-y-3 max-w-lg">
            ...
            <div className="space-y-1.5">
                {settingsItems.map((s, i) => (
                    <div key={i} className="bg-card border border-border rounded-xl px-4 py-3 flex items-center gap-3 hover:bg-accent/50 hover:border-primary/20 transition-all cursor-pointer group">
                        <div className="w-8 h-8 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><s.icon className="w-4 h-4 text-primary" /></div>
                        <div className="flex-1 min-w-0"><p className="text-[10px] font-semibold text-foreground">{s.label}</p><p className="text-[8px] text-muted-foreground">{s.sub}</p></div>
            ...
            <div className="flex min-h-[500px]">
                ...
                <div className="hidden sm:flex w-40 border-r border-border bg-muted/20 flex-col">
                    <div className="px-3 py-3 border-b border-border">
                        <div className="flex items-center gap-2">
                            <Image src="/logo.png" alt="OP" width={22} height={22} className="rounded-md object-contain" />
                            <span className="text-[10px] font-bold text-foreground">Origins Pillar</span>
                        </div>
                    </div>
                    ...
                    <div className="p-2 border-t border-border">
                        <div className="flex items-center gap-2 px-2 py-1.5">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-[9px] font-semibold text-foreground">Jane Doe</p>
                                <p className="text-[7px] text-muted-foreground">Admin</p>
                            </div>
                            <LogOut className="w-3 h-3 text-muted-foreground/40 cursor-pointer hover:text-muted-foreground" />
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
                        <div className="flex items-center gap-1.5 bg-accent border border-border rounded-lg px-2.5 py-1.5 w-52">
                            <Search className="w-3 h-3 text-muted-foreground/50" />
                            <span className="text-[9px] text-muted-foreground/50">Search anything...</span>
                        </div>
                        <div className="relative w-7 h-7 rounded-lg bg-accent border border-border flex items-center justify-center cursor-pointer hover:bg-accent/80">
