...
export async function POST(req: Request) {
    try {
        // Anti-DDoS: Secure the unauthenticated checkout route from API Amplification Attacks
        const ip = req.headers.get("x-forwarded-for") || "unknown-ip";
        const rateLimitResult = await rateLimitIP(ip); // Default 5 req / min

        if (!rateLimitResult.success) {
            return new NextResponse("Checkout rate limit exceeded. Please try again later.", { status: 429 });
        }
