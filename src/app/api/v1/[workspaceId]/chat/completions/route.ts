...
export const maxDuration = 60; // 60 seconds Next.js max duration limit
...
export async function POST(
    ...
    try {
        ...
        // Check rate limits
        const rateLimitResult = await checkRateLimit(gatewayKeyId, rateLimitRpm, rateLimitRpd);
        if (!rateLimitResult.success) {
            return NextResponse.json(
                { error: 'Rate limit exceeded for this Gateway Key.' },
        ...
        try {
            ...
            // Log failed request synchronously or asynchronously (bg)
        ...
        // Log request in background to avoid blocking API response
        prisma.usageRecord.create({
            data: {
                workspaceId: workspace.id,
                gatewayKeyId: gatewayKeyId,
        ...
        // Update provider last used in background
