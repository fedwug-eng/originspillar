...
const statusStyles: Record<string, string> = {
    ...
    "Backlog": "text-muted-foreground/50 bg-card border-border",
};
...
export default async function RequestsPage() {
    ...
    return (
        <div className="space-y-6">
            ...
            {requests.length === 0 ? (
                <div className="bg-card backdrop-blur-md border border-border rounded-2xl p-16 text-center">
                    <div className="w-16 h-16 rounded-2xl bg-card border border-border flex items-center justify-center mx-auto mb-4">
                        <FolderKanban className="w-7 h-7 text-muted-foreground/30" />
                    </div>
                    <p className="text-sm font-medium text-muted-foreground">No requests yet</p>
                    <p className="text-xs text-muted-foreground/50 mt-1">Create a new project to get started</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {requests.map((p) => (
                        <div
                            key={p.id}
                            className="bg-card backdrop-blur-md border border-border rounded-2xl p-5 hover:bg-accent hover:-translate-y-[2px] hover:shadow-xl hover:shadow-primary/[0.06] hover:border-border transition-all duration-300 ease-out cursor-pointer group block"
                        >
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground/80">{p.client.name}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mb-4">
                                <div className="flex items-center justify-between mb-1.5">
                                    <span className="text-xs text-muted-foreground/80">Progress</span>
