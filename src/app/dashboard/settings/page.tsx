            ...
            <div className="space-y-3">
                {sections.map((s, i) => (
                    <Link
                        ...
                        key={i}
                        className="bg-card backdrop-blur-md border border-border rounded-2xl p-5 flex items-center gap-4 hover:bg-white/[0.07] hover:-translate-y-[2px] hover:shadow-xl hover:shadow-primary/[0.06] hover:border-border transition-all duration-300 ease-out cursor-pointer group block"
                    >
                        ...
                        <div className="flex-1">
                            <h3 className="text-sm font-semibold text-foreground/80 group-hover:text-white transition-colors">{s.label}</h3>
                            <p className="text-xs text-muted-foreground/80">{s.description}</p>
                        </div>
                        <svg className="w-4 h-4 text-muted-foreground/30 group-hover:text-muted-foreground/70 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                    </Link>
                ))}
            </div>
        </div>
